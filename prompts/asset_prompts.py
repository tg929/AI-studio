...
3. 人物、场景、道具必须属于同一个项目世界，题材、时代、审美方向必须由 `style_bible.json` 和资产数据共同决定，不能无依据地固定到某一种世界观。
...
9. 若 `asset_registry.json` 中存在人物的 `visual_profile`、`costume_profile`、`visual_identity_lock`，必须优先使用这些结构化字段，而不是只依赖 `appearance_summary` 与 `costume_summary`。
...
- 必须把 `visual_profile` 与 `costume_profile` 里的关键信息自然融合进 prompt
- 必须明确强调：同一人物、同一服装、同一发型、同一配饰、同一身份
- 生成目标是“高质量且符合当前项目风格的人物设定图”的主体描述，而不是剧情海报或剧情镜头
- 要支持后续生成：左侧大幅近景肖像 + 右侧三个完整全身三视图
- 人物整体审美应偏“精致、统一、可重复、适合设定图主视觉”，并严格服从当前项目题材与风格，避免幼态Q版、扁平卡通、低幼感、粗糙速写感
...
- 若 `visual_identity_lock.required_features` 中存在强锚点，必须在 prompt 中明确体现
- 若 `visual_identity_lock.forbidden_drifts` 中存在高风险漂移方向，必须通过正向表述把人物锁在正确类别上，但不要把“禁止漂移”原句直接写进画面语言
- 若 `costume_profile.primary_color`、`hair_style`、`trim_details`、`accessories` 等字段为空或不明确，不要自行补出新的主色、新配饰或新发型细节
...
13. 人物气质要更接近“高质量项目一致人物设定图”，而不是粗糙动漫截图或低幼卡通角色卡。
14. 右侧三视图虽然由下游控制，但你的主体描述必须足够支撑同一角色/场景/道具在多个视角下保持稳定。
15. 人物 prompt 应尽量写出有利于“左大右三视图”设定图的高质量美术描述，例如发丝层次、布料材质、衣摆结构、镶边、暗纹、鞋履与配饰。
16. 人物 prompt 必须优先锁定性别、年龄段、服装类型、服装主色、发型长度、关键镶边与身份感，避免人物在下游被画错位。
17. 场景 prompt 应尽量写出能让同一地点多角度保持一致的空间锚点与布局关系。
18. 道具 prompt 应尽量写出能让同一道具多角度保持一致的轮廓、体量、结构与材质细节。
