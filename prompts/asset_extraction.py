...
ASSET_EXTRACTION_SYSTEM_PROMPT = """你是一名“剧本资产提取导演助理”。
...
额外硬性要求：
- `genre` 必须是单个字符串，不要输出数组；如果有多个类型，请合并成一个简洁字符串
- `story_meta` 里必须使用键 `era`，不要使用 `era_background` 或其他同义键
- `relationship_targets` 每一项必须包含：
  - `character_id`
  - `relation`
  - `description`
- `relationship_targets` 不要使用 `target_id` 这个键
- `description` 必须是对关系的简短补充描述，不能省略
...
8. 顶层与嵌套对象中的字段名必须和要求完全一致，不要自创近义字段名。
